...
Libmebo_brc_VP9::Libmebo_brc_VP9(LibMeboBrcAlgorithmID algo_id) : Libmebo_brc(LIBMEBO_CODEC_VP9,  algo_id)
{
	std::cout<<"try - Libmebo_brc_VP9 constructor \n";
...
LibMeboRateController* Libmebo_brc_VP9::init(LibMeboRateController* libmebo_rc,
...
LibMeboStatus Libmebo_brc_VP9::update_config(LibMeboRateController* rc,
...
LibMeboStatus Libmebo_brc_VP9::post_encode_update(LibMeboRateController* rc, uint64_t encoded_frame_size)
{
  LibMeboStatus status = LIBMEBO_STATUS_UNKNOWN;
...
LibMeboStatus Libmebo_brc_VP9::compute_qp(LibMeboRateController *rc, LibMeboRCFrameParams* rc_frame_params)
{
  LibMeboStatus status = LIBMEBO_STATUS_UNKNOWN;
...
LibMeboStatus Libmebo_brc_VP9::get_loop_filter_level(LibMeboRateController* rc, int* filter_level)
{
  LibMeboStatus status = LIBMEBO_STATUS_UNKNOWN;
