...
LibMeboRateController * libmebo_create_rate_controller (
  ...
  Libmebo_brc_factory::create(static_cast<LibMeboBrcAlgorithmID>(id));
  if (!brc)
    return nullptr;
...
LibMeboStatus libmebo_post_encode_update(LibMeboRateController* rc, 
    ...
    Libmebo_brc *brc = reinterpret_cast<Libmebo_brc *>(rc);
    return static_cast<LibMeboStatus>(brc->post_encode_update(rc, encoded_frame_size));
}
...
LibMeboStatus libmebo_get_loop_filter_level(LibMeboRateController* rc, 
                                                  int* filter_level) {
    Libmebo_brc *brc = reinterpret_cast<Libmebo_brc *>(rc);
    return static_cast<LibMeboStatus>(brc->get_loop_filter_level(rc, filter_level));
}
